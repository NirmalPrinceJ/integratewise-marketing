  ...
  return (
    <section className="py-16 md:py-24 bg-[#F3F4F6]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-3xl md:text-4xl font-extrabold text-[#0f1a15] text-center mb-10 md:mb-16">Why IntegrateWise is not just another integration tool</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-0 sm:border border-slate-200 rounded-3xl overflow-hidden bg-white shadow-xl">
          {columns.map((col, i) => (
            <div key={i} className={`p-6 md:p-8 ${col.highlight ? 'bg-[#059669] text-white sm:ring-4 ring-[#059669]/20 z-10 rounded-2xl sm:rounded-none' : 'bg-white text-slate-800'} ${!col.highlight ? 'border border-slate-200 sm:border-0 rounded-2xl sm:rounded-none' : ''}`}>
              <div className="mb-8">
